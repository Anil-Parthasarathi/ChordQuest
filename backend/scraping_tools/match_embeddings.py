...
# --- CONFIGURATION ---
BASE_PATH = Path("./backend/data/processed")
...
def step_2_cluster_and_index():
    ...
    cpu_index = faiss.IndexIVFFlat(quantizer, EMBEDDING_DIM, N_LIST, faiss.METRIC_INNER_PRODUCT)
    ...
    print(f"Index Ready. Vectors: {gpu_index.ntotal}")
    ...
    return labels, gpu_index

# ==========================================
# PHASE 3: MATCHING (Bridge Building)
# ==========================================
def step_3_match_musescore(work_ids, gpu_index):
    ...
    with tqdm(total=total_batches, desc="Matching") as pbar:
        for batch in reader.iter_batches(batch_size=QUERY_BATCH_SIZE):
            ...
            distances, indices = gpu_index.search(query, k=1)
